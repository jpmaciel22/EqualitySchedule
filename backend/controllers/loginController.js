...
exports.register = async (req, res, next) => {
  try {
    const { email, password, typeUser, nome, cpf, telefone, regiao, especificacao } = req.body;
    ...
    if (typeUser == 'medico') {
      ...
      const senhaEncrypt = await bcrypt.hash(password, 10);
      await Medico.create({ email, password: senhaEncrypt, nome, cpf, telefone, regiao, especificacao });
